...
private:
	...
	void makeWindow(const char * name) {makeSDLWindow(name);};
...
void SdlPreview::makeSDLWindow(char const *name, int selected_display)
{
	...
	SDL_GL_MakeCurrent(window, context); // Add this line
...
void SdlPreview::makeBuffer(int fd, size_t size, StreamInfo const &info, Buffer &buffer)
{
	...
	// PFNEGLQUERYDEVICEATTRIBEXTPROC eglQueryDeviceAttribEXT = NULL;

	// // Load them dynamically
	// eglQueryDeviceStringEXT = (PFNEGLQUERYDEVICESTRINGEXTPROC)
	// 	eglGetProcAddress("eglQueryDeviceStringEXT");

	// eglQueryDeviceAttribEXT = (PFNEGLQUERYDEVICEATTRIBEXTPROC)
	// 	eglGetProcAddress("eglQueryDeviceAttribEXT");

	// // Check they loaded
	// if (!eglQueryDeviceStringEXT || !eglQueryDeviceAttribEXT) {
	// 	fprintf(stderr, "Failed to load device query functions\n");
	// }
    // eglGetProcAddress("eglQueryDeviceAttribEXT");
	// for (int i = 0; i < numDevices; ++i) {
	// 	EGLDeviceEXT device=eglDevs[i];
	// 	EGLAttrib value;
	// 	if (eglQueryDeviceAttribEXT(device, EGL_DRIVER_UUID_EXT, &value)) {
	// 		printf("Driver UUID: %lld\n", (long long)value);
	// 	}
	...
	// 	NULL
	// );			
	// // egl_display_=SDL_GL_GetEGLDisplay();
	...
	// EGLDisplay alt_display = eglGetDisplay(EGL_DEFAULT_DISPLAY); // X11/Wayland default
	// if (alt_display == EGL_NO_DISPLAY) {
	// 	fprintf(stderr, "Failed to get EGLDisplay\n");
	// 	exit(1);
	// }

	// if (!eglInitialize(alt_display, NULL, NULL)) {
